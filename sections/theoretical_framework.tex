...
observations available when the forecast is issued. A causal integer-valued
predictor is selected from the explicitly restricted decision class
\begin{equation}
...
\(\mathbb{E}[E_t^2]<\infty\). Forecast information sets are causal and
nested:
...
\subsection{Exact DELB and Predictor-Dependent Refinement}
...
\begin{Theorem}[Exact discrete entropy prediction-error lower bound]
\label{thm:exact_delb}
Under Assumption~\ref{ass:general_regularity}, every causal integer-valued
predictor satisfies
\begin{align}
D_t^{(m)}
&\geq
...
to RQ1: target uncertainty and the integer lattice impose an MSE floor
...
by \(I(E_t;\mathcal F_{t-m})\). The proof is provided in
Appendix~\ref{app:proofs}. Boundary cases and the closed-form, temporal,
stationary, and vector consequences are stated below; their proofs remain in
the appendix.

To distinguish the DELB from minimum decision risk, define the optimal risk
within this same causal integer-valued predictor class by
\begin{equation}
...
Appendix~\ref{subsec:prior_bound_scope} compares the DELB with established
and recent neighboring bounds, including the explicit originality claims not
made here. Keeping that comparison and the supplementary corollaries in the
appendix allows the main text to focus on the implementable bound, its
attainability diagnostics, and added-information ordering.
...
MSE floor without integer support and squared-error loss; the lattice
...
Let \(\mathcal F_t^0\) be a baseline causal information set and let \(S_t\)
...
Appendix~\ref{app:proofs} gives the complete proofs, places the DELB relative
to established information-theoretic bounds, and states the supplementary
